...
def run_match(participant1, participant2):
    ...
    while winner not in (participant1, participant2):
        print(
            "Invalid input."
            "Please enter the name of the winning participant correctly.\n"
        )
        winner = input(f"Winner: ").strip().title()
...
def input_participants(tournament_id, tournament_title, size):
    ...
    while True:
        ...
        participants = [participant.strip().title()
                        for participant in participants]
        if len(participants) != int(size):
            print(
                f"\nInvalid number of participants."
                "Please enter {size} participants\n"
            )
            continue
        ...
        if "" in participants:
            print("\nInvalid input."
                  "Please enter the name of each participant\n")
            continue
        break
    print(
        f"\nPlease wait,"
...
def view_tournament(tournament_id, tournament_title):
    ...
    while True:
        ...
        if choice in ["1", "2", "3", "exit"]:
            ...
            print('\nInvalid input.'
                  'Please enter a valid option or type "Exit" to go back.\n'
                  )
...
def choose_participants(tournament_id, tournament_title, size):
    ...
    """
    print("Would you like to enter your own participants"
          "or use sample participants?\n")
...
def create_tournament():
    ...
    while True:
        ...
        if len(tournament_title) > 3 and len(tournament_title) < 50:
            ...
            print(
                "\nInvalid input."
                "Please enter a title between 4 and 50 characters.\n"
            )
    # Create the ID of the tournament
    tournament_id = tournament_title[0:3].upper()
    tournament_id += str(random.randint(100, 999))
    ...
    if choice == "1":
        ...
        while True:
            ...
            try:
                ...
                print('\nInvalid ID.'
                      'Please enter a valid ID or type "Exit" to go back\n')
        view_tournament(tournament_id, SHEET.worksheet(tournament_id).title)
    elif choice == "3":
        print("\nThank you for using Tournament Brackets\n")
        print("If you would like to use the app again,"
